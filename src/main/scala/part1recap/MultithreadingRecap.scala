...
object MultithreadingRecap extends App {
  ...
  val futures = Future {
    // long computation - runs on a different thread
    42
  }

  // callbacks
  futures.onComplete {
    case Success(42) => print("Success")
    case Failure(_) => println(("Failure"))
  }

  val aProcessedFuture = futures.map(_ + 1) // Future with value 43
  val aFlatFuture = futures.flatMap {
